...
// apexcraft命令的实现
function apexcraft(ctx: Context) {
  ctx.command('apexcraft', '查询Apex Legends的复制器信息')
  .action(async ({ session }) => {
    let page = await ctx.puppeteer.page();

    // 打开复制器工艺旋转的网页
    await page.goto('https://apexlegendsstatus.com/crafting-rotation', { waitUntil: 'networkidle0' });

    // 定位到复制器内容
    const selector = 'div.row > div.legpickrate > div.row';
    ...
    const screenshotPath = path.join(screenshotsDir, "Replicators crafting rotation.png");
